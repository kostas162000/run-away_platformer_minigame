...
{
    public float stabDistance = 1.0f;  // How far the knife moves forward during the stab
    public float stabSpeed = 8f;      // Speed of the stabbing motion
    public float rotationSpeed = 6f;  // Speed of the rotation before/after the stab
    public float attackRange = 3.5f;     // Maximum range for the hit detection raycast
    ...
    /*private void PerformHitDetection()
    {
        ...
        Vector3 rayStart = swordTip.position;
        
        // Draw a debug ray to see where it's going (visible in Scene View)
        Debug.DrawRay(rayStart, swordTip.forward * attackRange, Color.red, 1.0f);

        if (Physics.Raycast(rayStart, swordTip.forward, out hit, attackRange))
        {
